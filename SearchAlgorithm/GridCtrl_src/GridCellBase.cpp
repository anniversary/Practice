    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            */
			...
			{
				// XRadish * rect.TopLeft -> xxx
				CPoint pos = rect.CenterPoint();
				//pos.Offset(-nImageWidth-5, -nImageHeight/2);
				pos.Offset(-nImageWidth/2, -nImageHeight);
                pGrid->GetImageList()->Draw(pDC, GetImage(), pos, ILD_NORMAL);
			}
...
BOOL CGridCellBase::GetTextRect( LPRECT pRect)  // i/o:  i=dims of cell rect; o=dims of text rect
{
    if (GetImage() >= 0)
    {
        ...
        if (pImageList && pImageList->GetImageInfo( GetImage(), &Info))
        {
			...
			pRect->top += Info.rcImage.bottom-Info.rcImage.top;
        }
    }
